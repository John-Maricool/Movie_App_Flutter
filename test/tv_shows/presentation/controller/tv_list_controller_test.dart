...
import 'tv_list_controller_test.mocks.dart';

class TvListUsecaseTest extends Mock implements TvListUsecase {}

@GenerateMocks([TvListUsecaseTest])
void main() {
  late TvListController controller;
  late MockTvListUsecaseTest movieListUsecaseTest;
  final testResult = [
  ...
  setUp(() {
    movieListUsecaseTest = MockTvListUsecaseTest();
    controller = TvListController(usecase: movieListUsecaseTest);
  });
  ...
  group("Testing the movie list controller getmovies", () {
    ...
    test("Testing the getMovies function", () {
      when(movieListUsecaseTest.getTvList(1, any)).thenAnswer((invocation) {
      ...
      verify(movieListUsecaseTest.getTvList(1, any));
    });
